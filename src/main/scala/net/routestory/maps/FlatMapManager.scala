...
import net.routestory.browsing.Coordinator
...
class FlatMapManager(map: GoogleMap, mapView: View, displaySize: List[Int], coordinator: ActorSelection)(implicit ctx: ActivityContext, appCtx: AppContext)
  ...
  def add(chapter: Chapter) = {
    ...
    // create marker items
    val markerItems: Vector[MarkerItem] = chapter.elements.toVector flatMap {
      case m: Story.UnknownElement ⇒ Vector.empty
      case m: Story.Image ⇒ new ImageMarkerItem(m, m.timestamp) +: Vector.empty
      case m: Story.KnownElement ⇒ new SingleMarkerItem(m, m.timestamp) +: Vector.empty
    }
  ...
  private def doiEvaluate(markerItems: List[MarkerItem]) {
    ...
    val center = markerItems map { item ⇒
      ...
      case Nil ⇒ _center
      case l ⇒
        val target = l.minBy(_._2)._1
        //coordinator ! Coordinator.Cue(target.timestamp, Coordinator.Cue.Astronaut)
        p.toScreenLocation(target.location)
    }
